...
fun revMap (l, f) = List.foldl (fn (x, l) => f x :: l) [] l

fun concatRev l = List.foldl (op List.@) [] l

fun lconcat l = concatRev (rev l)
...
fun escapeString s =
 ...
 | _ => Char.toString c)
       ...
       case c of
          #"\n" => "\\\\\\\\n"
        | #"\t" => "\\\\\\\\t"
        | c => String.concat ["\\\\\\\\", Int.fmt (StringCvt.OCT) (Char.ord c)]) s
...
fun optionsToString (opts: 'a list, toString: 'a -> string, sep): string =
   concat ((*List. *)separate (List.map toString opts, concat [sep, " "]))
