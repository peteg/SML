(* Use to check everything builds. *)
use "platform/polyml/PolyMLNameSpace.sml";
use "ROOT.sml";
use "platform/polyml/polyml_ide/ROOT.sml";
...
val () =
    ...
    in
      Http_server.add_handler (Http_handlers.handle_filesystem {method=Method.GET, path=["dir"]}
                                                               {doc_root = [cwd, "platform", "polyml", "polyml_ide", "http_root"]})
    end

val () = Http_server.add_handler (Http_websockets.handle_websocket {method=Method.GET, path=["polyml_ide"]}
                                                                   Polyml_protocol.handle_polyml_ide)
